...
class AIWeatherService:
    """提供 AI 天氣分析、穿搭建議與互動對話的服務類別"""

    def __init__(self, gemini_api_key: Optional[str] = None, openai_api_key: Optional[str] = None, provider: str = "gemini"):
    ...
    def generate_weather_and_outfit_summary(
        ...
        if self.provider == "gemini" and self.gemini_api_key:
            ...
            if ai_res:
                ...
                return f"⚠️ **【Gemini API 呼叫失敗】**\n\n原因：`{self.last_error}`\n\n---\n\n" + self._generate_fallback_summary(city_name, current_weather, forecast_data, aqi_data, style_preference)
        elif self.provider == "openai" and self.openai_api_key:
            ai_res = self._call_openai_summary(context, city_name, style_preference)
            if ai_res:
                return ai_res
            else:
                return f"⚠️ **【OpenAI API 呼叫失敗】**\n\n原因：`{self.last_error}`\n\n---\n\n" + self._generate_fallback_summary(city_name, current_weather, forecast_data, aqi_data, style_preference)

        # 若未配置 API Key 或呼叫失敗，啟用智慧型規則引擎 (Fallback)
    ...
    def chat_response(
        ...
        if self.provider == "gemini" and self.gemini_api_key:
            ...
            if res:
                ...
                return f"⚠️ **【Gemini 機器人連線失敗】**\n\n詳細原因：`{self.last_error}`\n\n請檢查 API 金鑰有效性或網路連線。"
        elif self.provider == "openai" and self.openai_api_key:
            res = self._call_openai_chat(messages, context, city_name, style_preference)
            if res:
                return res
            else:
                return f"⚠️ **【OpenAI 機器人連線失敗】**\n\n詳細原因：`{self.last_error}`"
    ...
    # -------------------------------------------------------------
    # Gemini API 呼叫 (透過 REST API + x-goog-api-key Header 支援新舊所有金鑰)
    # -------------------------------------------------------------
        ...
        for model in models_to_try:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # 若 Header 方式失敗，嘗試相容 URL 參數方式
                    url_fallback = f"{url}?key={api_key}"
                    resp_fallback = requests.post(url_fallback, headers={"Content-Type": "application/json"}, json=payload, timeout=20)
                    if resp_fallback.status_code == 200:
                        ...
                        if candidates:
                            ...
                            if parts:
                                return parts[0].get("text", "")
                    self.last_error = f"Google API ({model}) HTTP {resp.status_code}: {resp.text}"
                    print(f"Gemini API ({model}) 回傳錯誤 [{resp.status_code}]: {resp.text}")
...
請用繁體中文、親切幽默且實用的口吻回答使用者的問題。回答需條理清晰，適當搭配 Emoji。"""
        ...
        api_key = self.gemini_api_key.strip()
        gemini_contents = []
        # Google Gemini 規範：第一則對話必須是 'user'，且 'user' 與 'model' 必須交替出現
        for msg in messages:
            role = "user" if msg["role"] == "user" else "model"
            if not gemini_contents and role == "model":
                # 跳過一開始的歡迎詞，避免第一個 turn 為 model 造成 400 錯誤
                continue
            if gemini_contents and gemini_contents[-1]["role"] == role:
                # 若連續同角色發言，合併至同一個 turn
                    ...
                    if resp_fallback.status_code == 200:
                        ...
                        if candidates:
                            ...
                            if parts:
                                return parts[0].get("text", "")
                    self.last_error = f"Google API ({model}) HTTP {resp.status_code}: {resp.text}"
                    print(f"Gemini Chat ({model}) 回傳錯誤 [{resp.status_code}]: {resp.text}")
            except Exception as e:
    ...
    # OpenAI API 呼叫
    # -------------------------------------------------------------
    def _call_openai_summary(self, weather_context: str, city_name: str, style_preference: str) -> Optional[str]:
...
"""
        ...
        try:
            ...
            print(f"OpenAI API 呼叫異常: {e}")
        return None

    def _call_openai_chat(self, messages: List[Dict[str, str]], weather_context: str, city_name: str, style_preference: str) -> Optional[str]:
        url = "https://api.openai.com/v1/chat/completions"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.openai_api_key}"
        ...
        try:
            ...
            print(f"OpenAI Chat 呼叫異常: {e}")
        return None

    # -------------------------------------------------------------
    # 智慧型規則引擎 (Fallback 備用生成，無須 API Key 也能完美運作)
    # -------------------------------------------------------------
    def _generate_fallback_summary(
        ...
        return f"""
> 💡 *小提示：目前為智慧規則生成模式。若配置 Gemini 或 OpenAI API Key，AI 機器人將提供更具個人化與時尚細節的建議！*
